...
// node-uuid这个库里通过typeof window是否是undefine 
// 来判断是web环境还是node环境，而我们原生模拟了一个不完整的window
// 会导致找不到window上的一个加密方法，从而报错
// 这里hack一下，调用前去掉，调用后再加回来
// @ts-ignore
// if (globalThis.AppModulePath && isSceneNative) {
//     // @ts-ignore
...
export function uuid(compress = true) {
    const uuid = Uuid.v4();
    return compressUuid(uuid, compress);
}
